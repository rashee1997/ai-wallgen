...
def manage_imagen_settings():
    ...
    while True:
        ...
        choice = get_validated_input(
            "Select an option (1-5, b)", ["1", "2", "3", "4", "5", "b"]
        )
        ...
        if choice == "1":
            ...
            print_info(
                f"Current setting: {user_prefs.imagen_settings.get('number_of_images', 1)}"
            )
            ...
            num_choice = get_validated_input(
                "Select number of images (1-4, b)", ["1", "2", "3", "4", "b"]
            )
            ...
            print_info(
                f"Current seed: {user_prefs.imagen_settings.get('seed') or 'Random (None)'}"
            )
            print_info(
                "Setting a specific seed allows you to reproduce the same image style."
            )
            ...
            model_choice = get_validated_input(
                "Select model version (1-2, b)", ["1", "2", "b"]
            )
            if model_choice == "_INTERRUPTED_":
                continue
            if model_choice == "b":
                continue
            models = {"1": "imagen-3.0-generate-002", "2": "imagen-3.0-generate-001"}
            ...
            print_info(
                f"Number of Images: {user_prefs.imagen_settings.get('number_of_images', 1)}"
            )
            print_info(
                f"Seed: {user_prefs.imagen_settings.get('seed') or 'Random (None)'}"
            )
            print_info(
                f"Model Version: {user_prefs.imagen_settings.get('model_version', 'imagen-3.0-generate-002')}"
            )
            print_info(
                f"Negative Prompt: {user_prefs.imagen_settings.get('negative_prompt', '') or 'None'}"
            )
            ...
            confirm = get_validated_input(
                "Are you sure you want to reset Imagen settings to defaults? (y/n)",
                ["y", "n"],
            )
