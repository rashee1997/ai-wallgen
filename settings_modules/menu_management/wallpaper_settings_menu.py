...
from ui_utils import (
    ...
    print_warning, get_validated_input
)
...
def manage_wallpaper_settings():
    ...
    while True:
        ...
        print_option("1", f"Automatically Set Wallpaper: {'Yes' if settings.get('auto_set') else 'No'}")
        print_option("2", f"Skip Preview: {'Yes' if settings.get('skip_preview') else 'No'}")
        print_option("3", f"Preview GUI Backend: {settings.get('gui_preview_backend', 'qt')}")
        ...
        if choice == "1":
            ...
            print_info(f"Skip Preview set to {'Yes' if settings['skip_preview'] else 'No'}")
        elif choice == "3":
            ...
            selection = get_validated_input(
                "Choose backend (1-3, or 'b' to cancel): ",
                list(opt_map.keys()) + ["b"]
            )
            if selection != "b":
