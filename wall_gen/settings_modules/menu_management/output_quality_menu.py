# settings_modules/menu_management/output_quality_menu.py
import re
# Updated import
...
def manage_output_quality_settings():
    ...
    # Ensure quality_settings exists
    ...
    while True:
        ...
        # Display current values safely
        current_res = user_prefs.imagen_settings.get("quality_settings", {}).get("resolution", "[Not Set]")
        current_detail = user_prefs.imagen_settings.get("quality_settings", {}).get("detail_level", "[Not Set]")
        current_render = user_prefs.imagen_settings.get("quality_settings", {}).get("rendering_quality", "[Not Set]")

        print_option("1", f"Resolution: {current_res}")
        print_option("2", f"Detail Level: {current_detail if current_detail else '[Not Set]'}")
        print_option("3", f"Rendering Quality: {current_render if current_render else '[Not Set]'}")
        print_option("b", "Back")
        ...
        if choice == "1":
            ...
            if res_choice == "6":
                while True:
                    ...
                    if re.match(r"^\d+x\d+$", custom_res):
                        # Ensure quality_settings dict exists before assigning
                        user_prefs.imagen_settings.setdefault("quality_settings", {})["resolution"] = custom_res
                ...
                # Ensure quality_settings dict exists before assigning
                user_prefs.imagen_settings.setdefault("quality_settings", {})["resolution"] = resolutions[res_choice]
            ...
            detail_levels = {
                ...
                "3": "high",
                "4": "ultra",
                "5": None, # Represent 'None' as Python None
            }

            # Ensure quality_settings dict exists before assigning
            user_prefs.imagen_settings.setdefault("quality_settings", {})["detail_level"] = detail_levels[detail_choice]
            print_success(
            ...
            # Ensure quality_settings dict exists before assigning
            user_prefs.imagen_settings.setdefault("quality_settings", {})["rendering_quality"] = quality_levels[quality_choice]
            print_success(
