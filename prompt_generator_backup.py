...
try:
    ...
    logging.warning(
        "google.generativeai module not found. Some features will be disabled."
    )
...
def flatten_settings(settings, parent_key="", sep=" - ", ignore_keys=None):
    ...
    ignore_keys = ignore_keys or set(
        ["negative_prompt"]
    )  # Negative prompt handled separately
    flattened = []
...
def generate_prompt_gemini(tags, user_prefs=None):
    ...
    try:
        ...
        if use_user_preferences:
            cache_key = (
                str(tags) + str(user_prefs.imagen_settings) + str(gemini_model_name)
            )
        else:
        ...
        if (
            ...
            and hasattr(user_prefs, "preferred_styles")
            and hasattr(user_prefs, "preferred_moods")
        ):
            style = (
                user_prefs.preferred_styles[0] if user_prefs.preferred_styles else None
            )
        ...
        depth_of_field = (
            camera_settings.get("depth_of_field") if camera_settings else None
        )
        focal_length = camera_settings.get("focal_length") if camera_settings else None
        shutter_speed = (
            camera_settings.get("shutter_speed") if camera_settings else None
        )
        ...
        time_of_day = (
            lighting_settings.get("time_of_day") if lighting_settings else None
        )
        lighting_type = (
            lighting_settings.get("lighting_type") if lighting_settings else None
        )
        light_source = (
            lighting_settings.get("light_source") if lighting_settings else None
        )
        light_quality = (
            lighting_settings.get("light_quality") if lighting_settings else None
        )
        artificial_sources = (
            lighting_settings.get("artificial_sources", []) if lighting_settings else []
        )

        # Extract composition settings for the prompt
        composition_settings = settings.get("composition_settings", {})
        technique = (
            composition_settings.get("technique") if composition_settings else None
        )
        camera_angle = (
            composition_settings.get("camera_angle") if composition_settings else None
        )
        visual_flow = (
            composition_settings.get("visual_flow") if composition_settings else None
        )
        depth_layering = (
            composition_settings.get("depth_layering") if composition_settings else None
        )
        focal_point = (
            composition_settings.get("focal_point") if composition_settings else None
        )
        perspective = (
            composition_settings.get("perspective") if composition_settings else None
        )
        ...
        location_type = (
            environment_settings.get("location_type") if environment_settings else None
        )
        atmospheric_effects = (
            environment_settings.get("atmospheric_effects", [])
        ...
        post_processing = (
            style_settings.get("post_processing", []) if style_settings else []
        )
        ...
        texture_quality = (
            detail_settings.get("texture_quality") if detail_settings else None
        )
        special_effects = (
            detail_settings.get("special_effects", []) if detail_settings else []
        )
        ...
        color_temperature = (
            color_settings.get("color_temperature") if color_settings else None
        )

        # Extract quality settings for the prompt
        quality_settings = settings.get("quality_settings", {})
        resolution = (
            quality_settings.get("resolution", "3840x2160")
            if quality_settings
            else "3840x2160"
        )
        rendering_quality = (
            quality_settings.get("rendering_quality") if quality_settings else None
        )
        aspect_ratio = (
            user_prefs.aspect_ratio if hasattr(user_prefs, "aspect_ratio") else "16:9"
        )
        ...
        digital_software = (
            digital_settings.get("software", "") if digital_settings else ""
        )
        digital_effects = (
            digital_settings.get("digital_effects", []) if digital_settings else []
        )

        game_engine = (
            game_engine_settings.get("engine_type", "") if game_engine_settings else ""
        )
        game_genre = (
            game_engine_settings.get("game_genre", "") if game_engine_settings else ""
        )
        game_shader = (
            game_engine_settings.get("shader_type", "") if game_engine_settings else ""
        )

        painting_medium = (
            medium_settings.get("painting_medium", "") if medium_settings else ""
        )
        ...
        illustration_style = (
            illustration_settings.get("style", "") if illustration_settings else ""
        )
        line_quality = (
            illustration_settings.get("line_quality", "")
            if illustration_settings
            else ""
        )

        abstract_composition = (
            abstract_settings.get("composition_type", "") if abstract_settings else ""
        )
        movement_type = (
            abstract_settings.get("movement_type", "") if abstract_settings else ""
        )

        material_type = (
            material_settings.get("material_type", "") if material_settings else ""
        )
        material_finish = (
            material_settings.get("finish", "") if material_settings else ""
        )
        ...
        if user_terms:
            enhanced_user_terms = set(
                enhance_negative_prompt(term) for term in user_terms
            )
            final_terms = enhanced_user_terms.union(default_terms)
            negative_prompt = ", ".join(
                sorted(list(final_terms))
...
"""
            ...
            if not gemini_api_key:
                ...
                return enforce_prompt_format(
                    prompt, resolution, aspect_ratio, negative_prompt
                )
            ...
            try:
                ...
                if response.parts:
                    ...
                    # Ensure proper formatting with resolution and aspect ratio
                    final_prompt = enforce_prompt_format(
                        enhanced_prompt_text, resolution, aspect_ratio, negative_prompt
                    )
        ...
        if hasattr(user_prefs, "preferred_styles") and hasattr(
        ...
        painting_medium = medium_settings.get("painting_medium", "")
        brushwork = medium_settings.get("brushwork", "")
        texture = medium_settings.get(
            "texture", ""
        )  # Note: 'texture' might conflict if also in detail_settings
        illustration_style = illustration_settings.get("style", "")
        line_quality = illustration_settings.get("line_quality", "")
        abstract_composition = abstract_settings.get("composition_type", "")
        movement_type = abstract_settings.get("movement_type", "")
        ...
        if not use_user_preferences and user_prefs is None:
            ...
            for medium, competing_mediums in art_mediums.items():
                if medium.lower() in custom_prompt.lower():
                    ...
                    logging.info(
                        f"Detected art medium: {medium}, added competing mediums to negative prompt"
                    )
                    break
...
"""
            ...
            enhancement_instructions = (
                ...
                + NO_PREFS_PROMPT_INSTRUCTIONS.format(
                    resolution=resolution, aspect_ratio=aspect_ratio
                )
            )
...
"""
            ...
            style = (
                user_prefs.preferred_styles[0]
                if hasattr(user_prefs, "preferred_styles")
                and user_prefs.preferred_styles
                else None
            )
            mood = (
                user_prefs.preferred_moods[0]
                if hasattr(user_prefs, "preferred_moods") and user_prefs.preferred_moods
                else None
            )
        ...
        style_settings = (
            settings.get("style_settings", {}) if "settings" in locals() else {}
        )
        art_movement = style_settings.get("art_movement") if style_settings else None
        style_era = style_settings.get("style_era") if style_settings else None
...
"""
        ...
        if not gemini_api_key:
            ...
            return enforce_prompt_format(
                custom_prompt, resolution, aspect_ratio, negative_prompt
            )
            ...
            if response.parts and len(response.parts) > 0:
                ...
                if len(prompt_parts) > 1:
                    ...
                    if custom_prompt.lower() not in main_prompt.lower():
                        logging.warning(
                            "Enhanced prompt doesn't contain original subject, prepending it"
                        )
...
def enforce_prompt_format(prompt, resolution, aspect_ratio, negative_prompt=""):
    ...
    if "avoid:" in prompt.lower():
        ...
        if len(parts) > 1:
            ...
            if resolution.lower() in neg_part.lower():
                neg_part = re.sub(
                    r"(?i)" + re.escape(resolution) + r"(?:\s+resolution)?",
                    "",
                    neg_part,
                )
            ...
            if aspect_ratio.lower() in neg_part.lower():
                neg_part = re.sub(
                    r"(?i)" + re.escape(aspect_ratio) + r"(?:\s+aspect\s+ratio)?",
                    "",
                    neg_part,
                )
    ...
    if resolution.lower() in clean_prompt.lower():
        clean_prompt = re.sub(
            r"(?i)" + re.escape(resolution) + r"(?:\s+resolution)?", "", clean_prompt
        )
...
def generate_random_style_mix(user_prefs=None):
    ...
    categories_to_use = (
        custom_style_categories if custom_style_categories else default_style_categories
    )
...
"""
        ...
        if response.text:
            ...
            terms = [
                term.strip().lower()
                for term in re.split(r",", enhanced_text)
                if term.strip()
            ]
            seen = set()
            unique_terms = []
            ...
            logging.warning(
                "Gemini returned empty response for negative prompt enhancement."
            )
